...
class TextRedirector:
    ...
    def write(self, string):
        def _append(s: str) -> None:
            tc = self.text_ctrl
            try:
                vpos = tc.GetScrollPos(wx.VERTICAL)
                vrng = tc.GetScrollRange(wx.VERTICAL)
                vth = tc.GetScrollThumb(wx.VERTICAL)
                at_bottom = (vpos + vth) >= (vrng - 2)
            except Exception:
                vpos = None
                at_bottom = True

            tc.AppendText(str(s).expandtabs(4))

            if at_bottom:
                tc.ShowPosition(tc.GetLastPosition())
            else:
                # Best-effort: keep current scroll position when user is reading above.
                try:
                    if vpos is not None:
                        tc.SetScrollPos(wx.VERTICAL, vpos)
                except Exception:
                    pass

        wx.CallAfter(_append, string)
...
class App(wx.Frame):
    ...
    def show_prev_figure(self, event):
        if not self.current_technique_panel or not self.current_technique_panel.figures:
            return
        self.current_technique_panel.current_figure_index = (
            self.current_technique_panel.current_figure_index - 1
        ) % len(self.current_technique_panel.figures)
    ...
    def reset_calculation(self, event):
        def reset_panel(panel):
            ...
            if hasattr(panel, "current_figure_index"):
                panel.current_figure_index = -1
            if hasattr(panel, "last_result"):
            ...
            if hasattr(panel, "vars_columnas"):
                panel.vars_columnas = {}
            if hasattr(panel, "vars_chemshift"):
                panel.vars_chemshift = {}
        ...
        self.current_technique_panel = active_panel
        self.btn_prev_figure.Enable(False)
        self.btn_next_figure.Enable(False)
    ...
    def save_results(self, event):
        with wx.FileDialog(
            ...
            current_page = self.technique_notebook.GetCurrentPage()
            current_panel = getattr(current_page, "inner_panel", current_page)
            result = getattr(current_panel, "last_result", None)
