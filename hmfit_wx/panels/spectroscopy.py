...
class SpectroscopyPanel(wx.Panel):
    ...
    def _build_left(self, parent: wx.Panel) -> None:
        ...
        grid_sheets.Add(wx.StaticText(parent, label="Spectra sheet:"), 0, wx.ALIGN_CENTER_VERTICAL)
        self.spectra_choice = wx.Choice(parent, choices=[])
        self.spectra_choice.Bind(wx.EVT_CHOICE, self._on_spectra_sheet_changed)
        grid_sheets.Add(self.spectra_choice, 1, wx.EXPAND)
        ...
        efa_box = wx.StaticBoxSizer(wx.HORIZONTAL, parent, "EFA")
        self.efa_chk = wx.CheckBox(parent, label="Enable")
        self.efa_eigs = wx.SpinCtrl(parent, min=0, max=200, initial=0)
        efa_box.Add(self.efa_chk, 0, wx.ALL | wx.ALIGN_CENTER_VERTICAL, 6)
        efa_box.Add(wx.StaticText(parent, label="Eigenvalues:"), 0, wx.ALL | wx.ALIGN_CENTER_VERTICAL, 6)
        efa_box.Add(self.efa_eigs, 0, wx.ALL | wx.ALIGN_CENTER_VERTICAL, 6)
        ...
        root.Add(efa_box, 0, wx.EXPAND | wx.LEFT | wx.RIGHT | wx.BOTTOM, 8)
    ...
    def _build_right(self, parent: wx.Panel) -> None:
        ...
        self.constants_grid.CreateGrid(0, 5)
        ...
        self.constants_grid.SetColLabelValue(4, "K")
    ...
    def _on_browse(self, _evt: wx.CommandEvent) -> None:
        ...
        self.spectra_choice.Set(self._sheets)
        self.conc_choice.Set(self._sheets)
        if self._sheets:
            self.spectra_choice.SetSelection(0)
            self.conc_choice.SetSelection(0)
            self._on_conc_sheet_changed(None)

    def _on_spectra_sheet_changed(self, _evt: Optional[wx.CommandEvent]) -> None:
        # Placeholder: could add axis/channel options later.
        return
    ...
    def _regenerate_k_grid(self) -> None:
        ...
        # bool format can be lost after recreate
        self.k_grid.SetColFormatBool(3)
    ...
    def _gather_inputs(self) -> SpectroscopyInputs:
        if not self._file_path:
            raise ValueError("No Excel file selected.")
        spectra_sheet = self.spectra_choice.GetStringSelection()
        conc_sheet = self.conc_choice.GetStringSelection()
        if not spectra_sheet or not conc_sheet:
            raise ValueError("Select spectra/conc sheets.")
        ...
        for r in range(n_params):
            ...
            fixed = as_bool_cell(self.k_grid.GetCellValue(r, 3))
        ...
        return SpectroscopyInputs(
            ...
            efa_enabled=bool(self.efa_chk.GetValue()),
            efa_eigenvalues=int(self.efa_eigs.GetValue()),
    ...
    def _run_processing(self, inputs: SpectroscopyInputs) -> Dict[str, Any]:
        from backend_fastapi.spectroscopy_processor import (
        ...
        return process_spectroscopy_data(
            ...
            spectra_sheet=inputs.spectra_sheet,
            ...
            efa_enabled=inputs.efa_enabled,
            efa_eigenvalues=inputs.efa_eigenvalues,
            modelo=inputs.model_grid,
            non_abs_species=inputs.non_abs_species,
    ...
    def _populate_constants(self, consts: List[Dict[str, Any]]) -> None:
        ...
        for r, c in enumerate(consts):
            ...
            self.constants_grid.SetCellValue(r, 4, str(c.get("K", "")))
